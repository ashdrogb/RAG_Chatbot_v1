...
LLM_MODEL = "google/flan-t5-base"                      # Fast, works on any laptop
# LLM_MODEL = "google/flan-t5-large"                   # Better quality, slower
# LLM_MODEL = "HuggingFaceH4/zephyr-7b-beta"           # Best chat (needs 16GB RAM)
# LLM_MODEL = "microsoft/Phi-3-mini-4k-instruct"       # Good balance (8GB RAM)
# LLM_MODEL = "mistralai/Mistral-7B-Instruct-v0.2"     # Best quality (16GB RAM)
...
def initialize_llm():
    ...
    try:
        # Check which type of model we're using
        if "flan-t5" in LLM_MODEL.lower() or "t5" in LLM_MODEL.lower():
            # T5 models use text2text-generation
            print("   Using text2text-generation pipeline (T5 model)")
            ...
            # Other models (Zephyr, Mistral, Llama, Phi) use text-generation
            print("   Using text-generation pipeline (Causal LM)")
            hf_pipeline = pipeline(
                "text-generation",
                model=LLM_MODEL,
                max_new_tokens=256,
            ...
            llm = CausalLMWrapper(hf_pipeline)
        ...
        llm = HuggingFacePipeline(pipeline=hf_pipeline)
        print("✅ LLM ready (using fallback model)!")
...
def build_rag_graph():
    ...
    def generate_answer(state: RAGState):
        ...
        if "flan-t5" in LLM_MODEL.lower() or "t5" in LLM_MODEL.lower():
            ...
            prompt = f"""Answer this question based on the Indian Budget documents provided.
...
Provide a clear, concise answer in 2-3 sentences:"""
            ...
            # Zephyr chat format
            prompt = f"""<|system|>
You are a helpful assistant that answers questions about Indian Budget documents.
Answer clearly and concisely based only on the provided context.</|system|>

<|user|>
Context:
{state['context'][:1500]}

Question: {state['question']}</|user|>
...
Answer the question based only on the following Indian Budget documents. Be concise and clear.
...
### Answer:"""
        ...
        cleanup_markers = ["Answer:", "<|assistant|>", "### Answer:", "A:"]
